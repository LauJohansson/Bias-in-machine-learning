...
six_cit_data = pd.read_csv('/restricted/s161749/G2020-57-Aalborg-bias/6 citizens/6_cit_data.csv')


# In[12]:


six_cit_data.drop(columns=['Unnamed: 0','Fall']) 
...
classified_df = pd.DataFrame([],columns=X.columns)
classified_df['y_true'] = []
classified_df['y_hat_binary'] = []
classified_df['y_hat_probs'] = []
...
for i in range(1,11):
   ...
   # 
    ...
    # Plot non-normalized confusion matrix
     ...
     ############ FOR WOMEN ################   
        ...
        for title, normalize in titles_options:
            disp = plot_confusion_matrix(classifier, X_test[X_test['Gender']==0],
        ...
        F1=2*TP/(2*TP+FP+FN)
        ACC=classifier.score(X_test[X_test['Gender']==0], y_test[y_test['Gender']==0]['Fall']) # mark gender
        yhat=np.mean(classifier.predict(X_test[X_test['Gender']==0]))
        yhat_prob=pd.DataFrame(classifier.predict_proba(X_test[X_test['Gender']==0]))[1].mean()
    ...
    ############ FOR MEN ################    
        ...
        for title, normalize in titles_options:
            disp = plot_confusion_matrix(classifier, X_test[X_test['Gender']==1],
        ...
        FN=disp.confusion_matrix[1][0]
        F1=2*TP/(2*TP+FP+FN)
        ACC=classifier.score(X_test[X_test['Gender']==1], y_test[y_test['Gender']==1]['Fall']) # mark gender
        yhat=np.mean(classifier.predict(X_test[X_test['Gender']==1]))
        yhat_prob=pd.DataFrame(classifier.predict_proba(X_test[X_test['Gender']==1]))[1].mean()
    ...
    ############ FOR 6 citizens ################    
        ...
        classified_df_local = six_cit_data.drop(columns=['Unnamed: 0','Fall'])
        bin_=classifier.predict(classified_df_local)
        prob_=classifier.predict_proba(classified_df_local)[:,1]
        classified_df_local['y_true'] = six_cit_data['Fall']
        classified_df_local['y_hat_binary']=bin_
        classified_df_local['y_hat_probs']=prob_
        classified_df = pd.concat([classified_df,classified_df_local])


# In[21]:


classified_df['model']='RF'
classified_df.to_csv('/restricted/s161749/G2020-57-Aalborg-bias/6 citizens/RF_predictions.csv')
...
classified_df.groupby('index').mean()
...
metricW_df['Mean_y_hat']=np.round(pd.DataFrame(yhat_list_W)*100,2)
metricW_df['Mean_y_hat_prob']=np.round(pd.DataFrame(yhat_prob_list_W)*100,2)
metricW_df['Gender']=0
...
metricM_df['Mean_y_hat']=np.round(pd.DataFrame(yhat_list_M)*100,2)
metricM_df['Mean_y_hat_prob']=np.round(pd.DataFrame(yhat_prob_list_M)*100,2)
metricM_df['Gender']=1
