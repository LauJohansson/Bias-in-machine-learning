...
X = fall_data.drop(columns=['Unnamed: 0']) # using all covariates in the dataset. ,'Ats_0'
...
def LFR_custom(df_train,lfr=None):
    ...
    df2_gender=df_train["Gender"].copy() #Gemmer bare gender # kan kommenteres ind hvis man vil køres test på gender!
    ...
    rp_df_pd = pd.concat([rp_df_pd,df2_gender],axis=1) # kan kommenteres ind hvis man vil køres test på gender!
...
for train_index, test_index in kf.split(X):
    ...
    X_train_rp,lfr = LFR_custom(X_train_temp)  
    X_test_rp  = LFR_custom(X_test_temp,lfr)


# In[91]:


X_train.shape,X_train_rp.shape # _rp "mister" fall og gender


# In[94]:


X_train.drop(columns=['Fall']).groupby('Gender').mean()
...
sns.histplot(x='BirthYear',data=X_train_rp,hue='Gender',kde=True,bins=30)
...
sns.histplot(x='LoanPeriod',data=X_train_rp,hue='Gender',kde=True,bins=30)
...
sns.histplot(x='NumberAts',data=X_train_rp,hue='Gender',kde=True,bins=30)
...
for i in range(1,11):
    ...
    for train_index, test_index in kf.split(X):
        ...
        X_train_rp,lfr = LFR_custom(X_train_temp,lfr=None)  
        X_test_rp,lfr  = LFR_custom(X_test_temp,lfr)
...
metricW_df['Gender']=0
metricW_df['Model']='LR' #change to correct model
...
metricM_df['Gender']=1
metricM_df['Model']='LR' #change to correct model
...
metric_df.to_csv('/restricted/s161749/G2020-57-Aalborg-bias/Plot_metrics/LFR/LR_gender.csv')
...
ACC_df.to_csv('/restricted/s161749/G2020-57-Aalborg-bias/Plot_metrics/LFR/LR_all.csv')
