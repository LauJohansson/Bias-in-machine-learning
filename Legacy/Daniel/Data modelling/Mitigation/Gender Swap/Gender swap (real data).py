...
for train_index, test_index in kf.split(X):
    ...
    X_train_temp['Gender']=(X_train_temp['Gender']-1)*(-1)
    
    X_train_swap=pd.concat([X_train,X_train_temp])
    y_train_swap=pd.concat([y_train,y_train_temp])
...
X_train_swap.Gender.value_counts()
...
for i in range(1,11):
    ...
    #### Flip fra den ene til den anden for at skifte mellem SVM, LR og RF ###
        ...
        #classifier = svm.SVC(kernel='rbf', C=1, random_state=0,class_weight='balanced',probability=True).fit(X_train_swap, y_train_swap['Fall'])
        #classifier = LogisticRegression(max_iter=1000,class_weight='balanced').fit(X_train_swap, y_train_swap['Fall'])
        classifier = RandomForestClassifier(random_state=1).fit(X_train_swap, y_train_swap['Fall'])
   ...
   # 
    ...
    ############ FOR All ################   
        ACC=classifier.score(X_test, y_test['Fall']) # mark gender
...
metric_df.to_csv('/restricted/s161749/G2020-57-Aalborg-bias/Plot_metrics/Gender Swap/RF_gender.csv')
...
ACC_df.to_csv('/restricted/s161749/G2020-57-Aalborg-bias/Plot_metrics/Gender Swap/RF_all.csv')
