...
fall_data = pd.read_csv('/restricted/s164512/G2020-57-Aalborg-bias/COMPAS/Data/COMPAS_dataset_OHE_std.csv')
...
X = fall_data.drop(columns=['Unnamed: 0','is_recid']) # using all covariates in the dataset. ,'Ats_0'
#X1 = pd.DataFrame(preprocessing.scale(X1),columns=X1.columns)
#X2 = pd.DataFrame(fall_data['Gender'])
#X = pd.concat([X1],axis=1)
y = fall_data[['race','is_recid']]
...
for i in range(1,11):
    ...
    for train_index, test_index in kf.split(X):
        print("Running model ",modelcounter)
    ...
    #### Flip fra den ene til den anden for at skifte mellem SVM, LR og RF ###
        ...
        classifier = svm.SVC(kernel='rbf', C=1, random_state=0,class_weight='balanced',probability=False).fit(X_train.drop(columns=['race']) , y_train['is_recid'])
        #classifier = LogisticRegression(max_iter=1000,class_weight='balanced').fit(X_train.drop(columns=['race']), y_train['is_recid'])
        #classifier = RandomForestClassifier(random_state=1).fit(X_train.drop(columns=['race']), y_train['is_recid'])
   ...
   # 
    ...
    # Plot non-normalized confusion matrix
     ...
     ############ FOR WOMEN ################   
        ...
        for title, normalize in titles_options:
            disp = plot_confusion_matrix(classifier, X_test[X_test['race']=="African-American"].drop(columns=['race']),
                                         y_test[y_test['race']=="African-American"]['is_recid'], 
        ...
        F1=2*TP/(2*TP+FP+FN)
        ACC=classifier.score(X_test[X_test['race']=="African-American"].drop(columns=['race']), y_test[y_test['race']=="African-American"]['is_recid']) # mark gender
        yhat=np.mean(classifier.predict(X_test[X_test['race']=="African-American"].drop(columns=['race'])))
        #yhat_prob=pd.DataFrame(classifier.predict_proba(X_test[X_test['race']=="African-American"].drop(columns=['race'])))[1].mean()
    ...
    ############ FOR MEN ################    
        ...
        for title, normalize in titles_options:
            disp = plot_confusion_matrix(classifier, X_test[X_test['race']=="Caucasian"].drop(columns=['race']),
                                         y_test[y_test['race']=="Caucasian"]['is_recid'], 
                                     display_labels=class_names,
                                     cmap=plt.cm.Blues, normalize=normalize)
        ...
        FN=disp.confusion_matrix[1][0]
        F1=2*TP/(2*TP+FP+FN)
        ACC=classifier.score(X_test[X_test['race']=="Caucasian"].drop(columns=['race']), y_test[y_test['race']=="Caucasian"]['is_recid']) # mark gender
        yhat=np.mean(classifier.predict(X_test[X_test['race']=="Caucasian"].drop(columns=['race'])))
        #yhat_prob=pd.DataFrame(classifier.predict_proba(X_test[X_test['race']=="Caucasian"].drop(columns=['race'])))[1].mean()
    ...
    ############ FOR All ################   
        ACC=classifier.score(X_test.drop(columns=['race']), y_test['is_recid']) # mark gender
...
metricW_df['race']="African-American"
metricW_df['Model']='SVM' #change to correct model
colsW = list(metricW_df.columns.values)
metricW_df = metricW_df[['race','TPR', 'FPR', 'TNR', 'FNR', 'ACC', 'Mean_y_hat', 'Model']]
metricW_df.mean()
...
metricM_df['race']="Caucasian"
metricM_df['Model']='SVM' #change to correct model
colsM = list(metricM_df.columns.values)
metricM_df = metricM_df[['race','TPR', 'FPR', 'TNR', 'FNR', 'ACC', 'Mean_y_hat', 'Model']]
metricM_df.mean()
...
metric_df.to_csv('/restricted/s164512/G2020-57-Aalborg-bias/Plot_metrics/COMPAS/SVM_gender.csv')
...
#ACC_df.to_csv('/restricted/s161749/G2020-57-Aalborg-bias/Plot_metrics/Dropping D/RF_all.csv')
...
print("african-american: \\textbf{",TPRW_mean,"} & \\textbf{",FPRW_mean,"} & \\textbf{",TNRW_mean,"} & \\textbf{",FNRW_mean,"} \\\ ")
print(f"& ({TPRW_low}-{TPRW_high}) & ({FPRW_low}-{FPRW_high}) & ({TNRW_low}-{TNRW_high}) & ({FNRW_low}-{FNRW_high})\\\ ")
print("caucasian: \\textbf{",TPRM_mean,"} & \\textbf{",FPRM_mean,"} & \\textbf{",TNRM_mean,"} & \\textbf{",FNRM_mean,"} \\\ ")
...
print("\\textbf{African-American}: & \\textbf{",ACCW_mean,"} \\\ ")
print(f"& ({ACCW_low}-{ACCW_high}) \\\ ")
print("\\textbf{Caucasian}: & \\textbf{",ACCM_mean,"} \\\ ")
