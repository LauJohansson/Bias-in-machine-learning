...
all_data_gender=all_data_gender.melt(id_vars=["race","Model"],var_name="Metric",value_name="Value")
...
plt.grid(axis="x")
sns.barplot(data=all_data_gender[(filter1)&(filter2)],x="Metric", y="Value",hue="Gender_string"  ,ci=95,dodge=True)#,palette=palette_custom)
...
palette_custom ={"African-American": "C0", "Caucasian": "C1"}
gender_order=["African-American","Caucasian"]
...
for i,v in enumerate(list_of_models):
    ...
    filter3=all_data_gender["Metric"]!="Mean_y_hat"
    filter4=all_data_gender["Metric"]!="Mean_y_hat_prob"
    filter5=all_data_gender["Metric"]!="Mean_y_target"
    filter6=all_data_gender["race"].isin(["African-American","Caucasian"])
    #ax[i].title.set_text(v,size=15)
    ax[i].set_title(v,size=15)
    ax[i].set_ylim([0, 1])
    ax[i].grid(axis='x')
    sns.barplot(data=all_data_gender[(filter1)&(filter2)&(filter3)&(filter4)&(filter5)&(filter6)],x="Metric",y="Value",hue="race",ax=ax[i],hue_order=gender_order,palette=palette_custom)
    ax[i].legend(title="Race")
    if i==0:
        ax[i].set(xlabel='')
...
sns.stripplot(data=newFrame[(filter1)&(filter2)&(filter3)],x="Metric",y="Relation",hue="Model",jitter=0.15,size=10,alpha=0.8,linewidth=1,
